...
public class DrawLineChart {
	...
	public static void main(String[] args) {
		...
		ArrayList<String> rowKs = null;
		ArrayList<String> columnKs=null;
		int colNum = 0;
//		args[0]="C:\\Users\\Administrator\\Desktop\\遗传算法数据\\运行结果\\minVals0001.log";
		try {
			BufferedReader br = new BufferedReader(new FileReader(new File(
					args[0])));
			al = new ArrayList<double[]>();
			rowKs = new ArrayList<String>();
			...
			while (true) {
				...
				colNum = vals.length;
				double[] one = new double[vals.length];
				for (int i = 0; i < vals.length; i++) {
					one[i] = Double.valueOf(vals[i]);
				}
				al.add(one);
			}
		...
		columnKs.add("AVG");
		data = al.toArray(new double[al.size()][colNum]);
		rowKeys = rowKs.toArray(new String[rowKs.size()]);
